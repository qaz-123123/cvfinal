...
def cal_accuracy(trained_model, test_data, is_cnn=True):
    with torch.no_grad():
        ...
        for idx, data in enumerate(test_data, 0):
            ...
            if is_cnn:
                ...
                processor_images = processor(images_denorm, return_tensors='pt')
                outputs = trained_model(processor_images.pixel_values)
...
if __name__ == '__main__':
    ...
    model1.classifier[2] = torch.nn.Linear(in_features=model1.classifier[2].in_features, out_features=10, bias=True)

    processor = ViTImageProcessor.from_pretrained("swin_transformer")
    model2 = SwinForImageClassification.from_pretrained("swin_transformer")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(trainloader, 0):
            ...
            images_denorm = (images * 0.5) + 0.5
            processor_images = processor(images_denorm, return_tensors='pt').to(device)

            outputs2 = model2(processor_images.pixel_values)
        ...
        accuracy_list2.append(cal_accuracy(model2, testloader, is_cnn=False))
